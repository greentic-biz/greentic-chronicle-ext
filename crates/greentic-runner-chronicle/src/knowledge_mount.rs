...
/// system path — the same convention greentic-runner-host uses for the
...
pub(crate) async fn attach(runtime: AgentRuntime) -> AgentRuntime {
...
fn embedding_expectation() -> Option<(String, usize)> {
...
async fn ingest_corpus(tenant: &TenantCtx, chunks: Vec<AwChunk>) {
    if chunks.is_empty() {
        return;
    }
    let Some(config) = build_config() else {
